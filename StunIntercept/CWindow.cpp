...
bool CWindow::RegisterWindowClass() {
    ...
    auto currentProcName = Helpers::GetCurrentProcessName();
    ...
    for (HWND hwnd = GetTopWindow(NULL); hwnd != NULL; hwnd = GetNextWindow(hwnd, GW_HWNDNEXT))
    {
        ...
        if (Helpers::GetProcessName(dwProcessId) != currentProcName)
            continue;
...
CWindow::CWindow(std::shared_ptr<CSharedComm> _sharedComm, std::shared_ptr<CStunChecker> _stunChecker, std::shared_ptr<CStunIntercept> _stunIntercept) : mSharedComm(_sharedComm), mStunChecker(_stunChecker), mStunIntercept(_stunIntercept)
{
    mGui = std::make_shared<CGui>(_sharedComm, _stunChecker, _stunIntercept);
}
